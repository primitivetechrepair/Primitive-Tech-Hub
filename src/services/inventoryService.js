...
addItem(input) {
  ...
  // keep your current UI behavior: newest item at top
  ...
  return item;
    ...
    // CSV upsert (your exact logic, centralized)
    upsertFromCsvRow(o) {
      const data = getData();
      data.inventory = data.inventory || [];
      ...
      if (existing) {
        ...
        addAudit?.("inventory_updated_csv", { itemID: existing.itemID, userAction: "csv_update" });
      } else {
        data.inventory.push({
          itemID,
          itemName: o.ItemName || "",
        ...
        addAudit?.("inventory_added_csv", { itemID, userAction: "csv_add" });
      }
...
adjustQty(itemID, delta) {
  ...
  addAudit?.("inventory_qty_adjusted", { itemID, userAction: "adjust_qty", delta });
...
removeItem(itemID) {
  ...
  data.inventory = (data.inventory || []).filter((x) => x.itemID !== itemID);
